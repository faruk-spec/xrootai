...
<head>
    ...
    <title>Verification Failed - {{ \App\Models\SystemSetting::get('general_chatbot_name', 'XrootAI') }}</title>
...
<body x-data="{ darkMode: localStorage.getItem('darkMode') === 'true' }" :class="{ 'dark-mode': darkMode }">
    ...
    <div class="auth-container">
        <div class="clay-card auth-card text-center" style="max-width: 460px; padding: 40px 30px;">
            <div style="width: 76px; height: 76px; border-radius: 50%; background: rgba(239, 68, 68, 0.15); color: #ef4444; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; font-size: 2.4rem;">
                ⚠️
            </div>
            ...
            @if(auth()->check())
                <form method="POST" action="{{ route('verification.resend') }}" style="margin-bottom: 14px;">
                    @csrf
                    <button type="submit" class="clay-btn clay-btn-primary" style="width: 100%; padding: 14px; font-size: 1rem;">
                        🔄 Resend Verification Code
                    </button>
                </form>
            @endif

            <a href="{{ route('login') }}" class="clay-btn clay-btn-secondary" style="display: block; width: 100%; padding: 12px; text-decoration: none; font-size: 0.95rem;">
                ← Back to Login
            </a>
        </div>
    </div>
